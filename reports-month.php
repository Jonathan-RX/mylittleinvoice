<?php
	...
	.graph{display:block;width:410px;height:210px;border:1px solid grey;padding:20px;font-family:verdana;font-size:10px;}
	...
	.graph .content li{float:left;margin-left:17px;position:relative;}
	.graph .content li hr{width:1px;height:180px;position:absolute;background:#eee;top:0;left:-5px;;margin-top:-180px;}
	.graph .content li i{display:block;position:absolute;width:10px;border:1px solid #888;background:#eee;bottom:15px;left:15px;}
    ...
    <div class="wrapper">
      ...
      <!-- Content Header (Page header) -->
        <section class="content-header">
          <h1>
            <?php echo $lang['monthly_report']; ?>
            <small><?php echo $lang['monthly_report_for']; ?> : 
            <?php 
	            ...
	            echo date('m',strtotime($year . '-' . $month .'-1')) . '/' . $year;
	            ?>
	            </small>
          </h1>
          <ol class="breadcrumb">
            <li><a href="#"><i class="fa fa-dashboard"></i> <?php echo $lang['home']; ?></a></li>
            <li class="active"><?php echo $lang['monthly_report']; ?></li>
          </ol>
        </section>
        ...
        <section class="content">
         ...
         <div class="box box-solid">
                ...
                <div class="box-body">
	                <form action="reports-month.php" method="POST">
		            <select name="month" class="form-control payment_date" style="width:120px;float:left;height:20px;padding:0;margin-right:5px;">
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3 col-sm-6 col-xs-6 text-center">
	                    <?php
		                    $nbr_customers = $bd->get_results('SELECT COUNT(*) AS nbr FROM ' . $bd->prefix . 'customers WHERE date_add LIKE "%/' . $month . '/' . $year . '"');
		                    ...
		                    $nbr_quotes = $bd->get_results('SELECT COUNT(*) AS nbr FROM ' . $bd->prefix . 'quotes WHERE date LIKE "%/' . $month . '/' . $year . '"');
		                    ...
		                    $nbr_convert_quotes = $bd->get_results('SELECT COUNT(*) AS nbr FROM ' . $bd->prefix . 'quotes WHERE date LIKE "%/' . $month . '/' . $year . '" AND invoice="true"');
		                    $nbr_convert_quotes = $nbr_convert_quotes[0]->nbr;
		                    $nbr_invoices = $bd->get_results('SELECT COUNT(*) AS nbr FROM ' . $bd->prefix . 'invoices WHERE date LIKE "%/' . $month . '/' . $year . '"');
		                    ...
		                    $payments = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'payments WHERE date LIKE "%/' . $month . '/' . $year . '"');
		                    $nbr_payments = 0;
		                    foreach($payments as $p){
			                    $nbr_payments += $p->amount;
		                    }
		                    $total_payments = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'payments WHERE date LIKE "%/' . $year . '"'); 
		                    $all_payments = 0;
		                    foreach($total_payments as $p){
			                    $all_payments += $p->amount;
		                    }
		                    $invoices = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'invoices WHERE date LIKE "%/' . $month . '/' . $year . '"');
		                    $total_taxes = 0;
		                    $total = 0;
              ...
              <div class="box box-solid">
                ...
                <div class="box-body">
                  ...
                  <div class="graph evol_invoice">
	                <?php 
		                $date = $year . '-' . $month . '-1';
		                $date_1 = date('m/Y',strtotime($date . ' -1 month'));
		                $date_2 = date('m/Y',strtotime($date . ' -2 month'));
		                $date_3 = date('m/Y',strtotime($date . ' -3 month'));
		                $date_4 = date('m/Y',strtotime($date . ' -4 month'));
		                $date_5 = date('m/Y',strtotime($date . ' -5 month'));
		                
	                ?>
	                  <ul class="title">
		                  <?php 
			                  $t0 = $bd->get_results('SELECT COUNT(*) AS nbr FROM ' . $bd->prefix . 'invoices WHERE date LIKE "%/' . $month . '/' . $year . '"');
			                  $t0 = $t0[0]->nbr;
	                  ...
	                  <ul class="content">
		                  <?php
				                ...
				                echo '<li title="' . $month . '/' . $year . '">' . $month . '/' . $year . '<hr /><i style="height:' . ((180*$t0/$tmax)-15) . 'px;"><b>' . $t0 .'</b></i></li>';
			                   ?>
	                  </ul>
                  </div><!-- Stop Graph -->
                  <br />
                  ...
                  <div class="graph evol_invoice">
	                ...
	                ?>
	                  <ul class="title">
		                  <?php 
			                  $tp0 = $bd->get_results('SELECT * FROM ' . $bd->prefix . 'invoices WHERE date LIKE "%/' . $month . '/' . $year . '"');
			                  $t0 = 0;
			                  foreach($tp0 as $r){
				                  $total_taxes = 0;
			                    foreach(unserialize($r->products) as $p){
